...
- Caffeine caching for market data (`CacheConfig`): `marketChart` (15s TTL, 200 entries, keyed by symbol+interval+limit) and `marketTicker` (3s TTL, 50 entries, keyed by symbol), so concurrent browser tabs polling the same view do not multiply upstream Binance calls.
